...
function saveUser(username) {
  localStorage.setItem("namulUser", username);
}
function getUser() {
  return localStorage.getItem("namulUser") || null;
}
function clearUser() {
  localStorage.removeItem("namulUser");
}
...
async function login(username, password) {
  ...
  if (data.success) {
    saveUser(data.username || username);
    updateUI();
  } else alert(data.msg);
}
loginBtn.onclick = () => {
...
document.getElementById("btnCloseGameModal").onclick = () => gameModal.style.display="none";
